...
test('DHT: explicitly set nodeId', function (t) {
...
test('DHT: `ping` query send and response', function (t) {
  t.plan(2)
...
test('DHT: `find_node` query for exact match (with one in table)', function (t) {
  t.plan(3)
...
test('DHT: `find_node` query (with many in table)', function (t) {
  t.plan(3)
  var dht1 = new DHT({ bootstrap: false })
  var dht2 = new DHT({ bootstrap: false })
...
test('DHT: `get_peers` query to node with *no* peers in table', function (t) {
  t.plan(4)
  var dht1 = new DHT({ bootstrap: false })
  var dht2 = new DHT({ bootstrap: false })
...
test('DHT: `get_peers` query to node with peers in table', function (t) {
  t.plan(4)
...
test('DHT: `announce_peer` query with bad token', function (t) {
  t.plan(2)
  var dht1 = new DHT({ bootstrap: false })
  var dht2 = new DHT({ bootstrap: false })
...
test('DHT: `announce_peer` query gets ack response', function (t) {
  t.plan(5)
